...
if smdaPilot.itinerary.settings_z_stack_upper_offset(k) - smdaPilot.itinerary.settings_z_stack_lower_offset(k) > 0
    ...
    for a = 1:length(zstack)
        ...
        move_z(zstack(a));
        smdaPilot.snap;
    smdaPilot.itinerary.database_filenamePNG = sprintf('%s_s%d%s_w%d%s_t%d_z%d.tiff',smdaPilot.itinerary.group_label{i},j,strcat(smdaPilot.itinerary.position_label{j},''),smdaPilot.itinerary.settings_channel(k),smdaPilot.itinerary.channel_names{smdaPilot.itinerary.settings_channel(k)},smdaPilot.t,smdaPilot.database_z_number);
